...
/**
 * Staggered card / item — for grid sections (projects, skills, etc).
 */
export const staggerItemVariants = (delay: number = 0): Variants => ({
  hidden: { opacity: 0, y: 20 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.5, delay, ease: transitionEasing },
  },
});
